...
public class No {
	...
	// RETORNA O O SUCESSOR MAX DESSE ESTADO
	public int[][] getSucessorMax() {
		return this.sucessorMax;
	}
		...
		for (int coluna = 0; coluna < this.estado.length; coluna++) {
			for (int linha = 0; linha < this.estado.length; linha++) {
				if (this.estado[linha][coluna] == 1) {
					// ACHADO A RAINHA, CALCULO SEMPRE OS ATAQUES DA POSICAO DELA PARA A DIREITA DO
					// TABULEIRO
	...
	// PROCURA UM SUCESSOR MAX E MIN PARA ESSE ESTADO (ESTADO COM MAIOR E MENOR
	// NUMERO DE ATAQUES)
	private void sucessorMax() {
		...
		// ORDENO OS SUCESSORES
		ordenaCrescenteAtaque(sucessores);

		// VERIFICO SE TEM MAIS DE UM MÍNIMO PARA EFETUAR O SORTEIO DE QUAL IRÁ PARA O
		// SUCESSOR MAX
		int contador = 1;
		for (int i = 0; i < this.estado.length - 1; i++) {
			if (ataquesTabuleiro(sucessores.get(i)) == ataquesTabuleiro(sucessores.get(i + 1))) {
				contador++;
			}
		}
		...
		// ATRIBUTO SUCESSOR RECEBE A REFERENCIA DE UM MINIMO ALEATORIO
		this.sucessorMax = sucessores.get(gerador.nextInt(contador));
	...
	// MÉTODO QUE RECEBE UMA MATRIZ E UMA COORDENADA DESSA MATRIZ E RETORNA O NUMERO
	// DE ATAQUES NA LINHA, COLUNA, DIAGONAL PRINCIPAL E SECUNDARIA
	...
	public int ataques(int[][] matriz, int linha, int coluna) {
		...
		// VERIFICO NA LINHA (só verifico da coluna em diante, evitando que calcule dois
		// ataques de um mesmo par de rainhas )
		...
		// COPIO OS VALORES DA LINHA E DA COLUNA PARA NAO PERDER OS ORIGINAIS NAS
		// INCREMENTACOES FEITAS ABAIXO
		int coplin = linha;
		int copcol = coluna;
		...
		// DECREMENTO 4, POIS VERIFICO 4 LUGARES (LINHA, COLUNA, DIAGONAL PRINCIPAL
		// BAIXO, DIAGONAL SECUNDARIA CIMA) E EM TODAS A INCREMENTACAO CONTA A PROPRIA
		// RAINHA
		ataque = ataque - 4;
	...
	// MÉTODO QUE ORDENA UMA LISTA DE FORMA DECRESCENTE PELO CUSTO (PARA QUE QUANDO
	// SEJA INSERIDA NA BORDA, FIQUE DE FORMA CRESCENTE)
